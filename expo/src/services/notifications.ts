...
export async function rescheduleNotifications(state: AppState, devFastLegs: boolean): Promise<void> {
  ...
  if (state.settings.notifyArrival) {
    ...
    if (state.phase === 'traveling' && state.journey.arrivalAt > now) {
      try {
        await Notifications.scheduleNotificationAsync({
          identifier: ARRIVAL_NOTIF_ID,
          content: {
            title: 'You have arrived',
            body: 'A new waymark waits. Open the app to draw a card.',
          },
          trigger: {
 ...
 * 3. iOS: the first schedule will silently do nothing if the user has not
 *    granted permission. We call requestNotificationPermission() at app
 *    startup (in _layout) to prompt early. If the user denies, notifications
 *    simply won't show — the app still works.
